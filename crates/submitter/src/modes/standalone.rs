...
/// Standalone mode: Only submits transactions (listens to NATS and submits)
...
impl StandaloneMode {
    pub async fn new(
        ...
        info!(
            "Initializing Standalone mode - NATS: {}",
            match &config.mode {
                crate::service::SubmitterMode::Standalone { nats_url, .. } => nats_url,
                _ => "not configured",
            }
        );
        ...
        let (nats_url, consumer_name) = match &config.mode {
            crate::service::SubmitterMode::Standalone { nats_url, consumer_name } => {
    ...
    pub async fn run(mut self) -> Result<()> {
        info!("Starting Standalone mode - listening for transactions to submit");
