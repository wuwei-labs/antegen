...
impl NatsConsumer {
    pub async fn new(url: &str, consumer_name: &str, stream_name: Option<&str>) -> Result<Self> {
...
impl TransactionSource for NatsConsumer {
        ...
        if let Some(ref mut stream) = self.message_stream {
            ...
            match tokio::time::timeout(Duration::from_millis(100), stream.next()).await {
                Ok(Some(msg_result)) => {
                    match msg_result {
                        Ok(msg) => {
                            // Parse transaction from message using Anchor deserialization
                            match BuiltTransaction::deserialize(&mut &msg.payload[..]) {
                                Ok(tx) => {
                                ...
                                Err(e) => {
                                    ...
                                    msg.ack().await.map_err(|e| {
                                        anyhow::anyhow!("Failed to ack bad message: {}", e)
                                    })?;
                                    Ok(None)
                                }
                            }
    ...
    async fn nack(&mut self, tx_id: &str) -> Result<()> {
        ...
        if self.pending_messages.remove(tx_id).is_some() {
            debug!(
                "Not acknowledging transaction (will be redelivered): {}",
                tx_id
            );
        }
        Ok(())
